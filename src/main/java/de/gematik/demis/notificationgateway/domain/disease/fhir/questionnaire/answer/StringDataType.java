...
class StringDataType implements DataType<StringType> {

  @Override
  public StringType toFhir(QuestionnaireResponseAnswer answer) {
    return new StringType(getValue(answer));
  }

  @Override
  public boolean test(QuestionnaireResponseAnswer answer) {
    return StringUtils.isNotBlank(answer.getValueString());
  }

  private String getValue(QuestionnaireResponseAnswer answer) {
    return Objects.requireNonNull(answer.getValueString(), "string value not set");
  }
}
